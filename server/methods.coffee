...
autogenerateDocumentIncidents = (source, options={}) ->
  if source.reviewed
    throw new Meteor.Error("annotation-error", "Incidents cannot be autogenerated for reviewed articles.")
...
getArticleEnhancements = (article, options=null) ->
  ...
  if options.reprocess and article.enhancements?.source
    ...
    post_data.url = formatUrl(article.url)
  else
    Meteor.Error("InvalidArticle", "Content or a URL must be specified")
  return new Promise((resolve, reject) ->
    HTTP.post(Constants.GRITS_URL + "/api/v1/public_diagnose", {
      params:
        api_key: Constants.GRITS_API_KEY
      data: post_data
    }, (err, result)->
...
Meteor.methods
  ...
  getArticleEnhancementsAndUpdate: (articleId, options={}) ->
    ...
    priorEnhancements = dbArticle.enhancements
    if priorEnhancements and not options.reprocess
      if priorEnhancements.processingStartedAt
        # If the processing started less than 100 seconds ago do not resubmit
        # the aritcle.
        if (new Date() - priorEnhancements.processingStartedAt) < 100000
          return priorEnhancements
      else if not priorEnhancements.error
        return priorEnhancements

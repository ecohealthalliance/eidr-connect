...
export createIncidentReportsFromEnhancements = (enhancements, options={}) ->
  ...
  incidents = incidents.concat((
    ...
    incident.locations = [incident.location]
    if incident.type == 'caseCount'
      incident.cases = incident.value
      incident.dateRange.type = 'precise'
    else if incident.type == 'cumulativeCaseCount'
      incident.cases = incident.value
      incident.dateRange.cumulative = true
      incident.dateRange.type = 'day'
    else if incident.type == 'deathCount'
      incident.deaths = incident.value
      incident.dateRange.type = 'precise'
    else if incident.type == 'cumulativeDeathCount'
      incident.deaths = incident.value
      incident.dateRange.cumulative = true
      incident.dateRange.type = "day"
    ...
    incident.accepted = Boolean(acceptByDefault)
    incident
  )
